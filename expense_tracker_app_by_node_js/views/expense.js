...
function addexpense(e){
    ...
    axios.post('http://54.252.183.134:8000/expense/add-expense',expense,{headers:{'Authorization':token}})
...
async function getAllExpanses(token,page){
    try{
        ...
        const res= await axios.get(`http://54.252.183.134:8000/expense/display-expense?page=${page}&entries=${entries}`,
...
async function getPaginated(page){
    try{
        ...
        entries = parseInt(entries);
        let res = await axios.get(`http://54.252.183.134:8000/expense/display-expense?page=${page}&entries=${entries}` ,
        { headers :{"Authorization":token}});
        item_list.innerHTML = "";
        for(var i=0;i<res.data.expenses[0].length;i++){
            showexpenses(res.data.expenses[0][i])
...
function remove_expense(e){
    ...
    if(e.target.classList.contains('delete')){
        if(confirm('Are you sure?')){
            ...
            axios.get("http://54.252.183.134:8000/expense/display-expense",{headers:{'Authorization':token}})
            .then((res)=>{
                for(var i=0;i<res.data.expenses[0].length;i++){
            ...
            .then(()=>{
                axios.delete(`http://54.252.183.134:8000/expense/delete-expense/${id}`,{headers:{'Authorization':token,'amount':selected_amt}})
                .then((res)=>console.log(res))
                .catch((err)=>console.log(err))
            }
            )
...
document.getElementById('rzp-button').onclick= async function(e){
    const token=localStorage.getItem('token')
    const response=await axios.get('http://54.252.183.134:8000/purchase/premiummembership',
    {headers:{'Authorization':token}})
    var options={
        "key":response.data.key_id,
        "order_id":response.data.order.id,
        "handler":async function(response){
            await axios.post('http://54.252.183.134:8000/purchase/updatetransactionstatus',{
...
document.querySelector('.leaderboard').onclick=()=>{
    axios.get("http://54.252.183.134:8000/expense/show-leaderboard").then((res)=>{
...
document.getElementById("download_expenses").addEventListener("click", async function(){
    try{
        const token=localStorage.getItem('token')
        const response=await axios.get("http://54.252.183.134:8000/expense/download",
        {headers:{'Authorization':token}})
        const downloaded_history=await axios.get("http://54.252.183.134:8000/expense/get-history-data",
        {headers:{'Authorization':token}})
